...
import catppuccinDarkTheme from "./assets/themes/catppuccin-mocha.css?raw";
import catppuccinLightTheme from "./assets/themes/catppuccin-latte.css?raw";
...
export default {
  ...
  methods: {
    ...
    buildDashboard: async function () {
      ...
      let config;
      try {
        config = await this.getConfig(window.location.hash.substring(1));
      } catch (error) {
        console.log(error);
      ...
      // add Theme catppuccinTheme
      let style = document.createElement("style");
      style.appendChild(document.createTextNode(catppuccinLightTheme));
      style.appendChild(document.createTextNode(catppuccinDarkTheme));
      document.head.appendChild(style);
    },
    getConfig: function (ns = "") {
      // create reqPath from endpoint.api_endpoint
      let reqPath = "/api/v1/namespaces";
      return fetch(reqPath).then((response) => {
        ...
        return response
          ...
          .then(function (config) {
            ...
            for (const namespace of config.data) {
              if (ns !== "" && ns !== namespace.name) {
                break;
              }
              const items = [];
